...
export type ElevationPrimitiveKind =
  ...
  | 'aid';       // 作図の補助（補助線・目印）。部材ではない (= E-8-v5c)
...
export type ElevationPrimitive =
  /** width は screen px。widthGrid (= E-8-v2h) があれば max(width, widthGrid×px/グリッド) を使う
   *  ＝平面と同じ実寸比で太らせつつ、縮小時も width を下回らない（線に潰れない）。 */
  | { kind: 'line'; x1: number; y1: number; x2: number; y2: number; stroke: string; width: number; widthGrid?: number; dash?: number[]; opacity?: number; meta?: ElevationPrimitiveMeta }
  ...
  | { kind: 'circle'; x: number; y: number; r: number; rGrid?: number; fill: string; stroke?: string; strokeWidth?: number; opacity?: number; meta?: ElevationPrimitiveMeta }
