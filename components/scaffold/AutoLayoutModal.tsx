...
import { isBandHonored, proposeClosingBands, type ClosingBandProposals } from '@/lib/konva/autolayout/loopFit';
...
export default function AutoLayoutModal({ onClose, onOpenScaffoldStart }: Props) {
  ...
  // S-2f-d-2/改修: 範囲離れが帯不履行(帯外離れで割った)のとき、帯を守れる方向別の提案帯を出すダイアログ。
  const [showBandProposal, setShowBandProposal] = useState<{ orig: { lo: number; hi: number }; props: ClosingBandProposals } | null>(null);
  ...
  const handleCalc = (bandOverride?: { lo: number; hi: number; mode: 'center' | 'lower' }) => {
    ...
    if (bothCascadeReady) {
      ...
      // S-2f-d改修: 帯不履行(採用離れが帯外＝この帯では割り切れない)なら、計算入口で1回だけ方向別の
      //   提案帯を出す。center のみ・提案[再計算]からの呼び出し(bandOverride)では再提案しない。配置は継続(非後退)。
      if ((effBand.mode ?? 'center') === 'center' && !bandOverride && !isBandHonored(res, effBand).honored) {
        const props = proposeClosingBands(
        ...
        if (props.expandUp || props.expandDown || props.expandBoth) {
          setShowBandProposal({ orig: { lo: effBand.lo, hi: effBand.hi }, props });
        }
      }
      ...
      // S-5b-2: cascade 降順の最初の未解決セグメントへ（N=2 では 2F 優先→1F と同順）。
      const firstUnresolved = flattenFocusList(res).find(x => !x.seg.isLocked && !x.seg.isAutoProgress);
      setActiveEdge(firstUnresolved
        ? { floor: firstUnresolved.floor, index: firstUnresolved.edgeIndex, segmentIndex: firstUnresolved.seg.segmentIndex }
        : null);
      return;
    }
  ...
  const handleSequentialSelect = (
    ...
    // Phase H-3d-2 Stage 5 Part C + 修正A + B1/B2 / S-5b-2: floor===2/===1 を融合
    if (bothCascadeReady) {
      const key = `${edgeIndex}-${segmentIndex}`;
      // record キーは floor 直用（both では primaryFloor=2/subFloor=1 と floor が恒等）。
      if (floor !== topFloor && !layoutByFloor) return; // 旧 floor===1(下位階) 枝のガード
      const newSelections = { ...(bothmodeSelectionsByFloor[floor] ?? {}), [key]: candIdx };
      setBothmodeSelectionsByFloor(prev => ({ ...prev, [floor]: newSelections }));

      // S5-d: 変更後は cascade で全階一括再計算（下位階は上位階結果に追従）
      const res = computeCascadeLayout(
        cascadeInput.buildings,
        cascadeInput.distances,
        cascadeInput.topStart!, enabledSizes, priorityConfig,
        { ...bothmodeSelectionsByFloor, [floor]: newSelections },
        bothmodeAdjustmentsByFloor,
        band,
      );
      setLayoutByFloor(res);
      ...
      // 次の未解決セグメントへ（cascade 降順 focusList で現在位置以降を探索・floor 跨ぎ含む）。
      //   N=2 では「2F 内 next → 無ければ 1F 先頭」と同順。
      const flat = flattenFocusList(res);
      const curArrIdx = flat.findIndex(
        x => x.floor === floor && x.edgeIndex === edgeIndex && x.seg.segmentIndex === segmentIndex,
      );
      const next = curArrIdx >= 0
        ? flat.slice(curArrIdx + 1).find(x => !x.seg.isLocked && !x.seg.isAutoProgress)
        : undefined;
      setActiveEdge(next
        ? { floor: next.floor, index: next.edgeIndex, segmentIndex: next.seg.segmentIndex }
        : null);
      return;
    }
  ...
  const handleSequentialBack = () => {
    ...
    if (bothCascadeReady) {
      ...
      // S-5b-2: cascade 降順 focusList で「現在位置より前の未解決(非autoProgress)」へ戻る。
      //   N=2 では「1F 内 prev → 無ければ 2F 末尾」「2F 内 prev → 無ければ no-op」と同順。
      if (!layoutByFloor) return;
      const flat = flattenFocusList(layoutByFloor);
      const curArrIdx = flat.findIndex(x => x.floor === activeEdge.floor && x.edgeIndex === activeEdge.index);
      const prevEntry = curArrIdx > 0
        ? flat.slice(0, curArrIdx).reverse().find(x => !x.seg.isAutoProgress)
        : undefined;
      if (!prevEntry) return;

      const pf = prevEntry.floor;
      const newSelections = stripEdge((bothmodeSelectionsByFloor[pf] ?? {}), prevEntry.edgeIndex);
      const newAdjustments = stripEdge((bothmodeAdjustmentsByFloor[pf] ?? {}), prevEntry.edgeIndex);
      setBothmodeSelectionsByFloor(prev => ({ ...prev, [pf]: newSelections }));
      setBothmodeAdjustmentsByFloor(prev => ({ ...prev, [pf]: newAdjustments }));

      // S5-d cascade で全階再計算（戻し後、下位階も追従）
      const res = computeCascadeLayout(
        cascadeInput.buildings,
        cascadeInput.distances,
        cascadeInput.topStart!, enabledSizes, priorityConfig,
        { ...bothmodeSelectionsByFloor, [pf]: newSelections },
        { ...bothmodeAdjustmentsByFloor, [pf]: newAdjustments },
        band,
      );
      setLayoutByFloor(res);
      ...
      setActiveEdge({ floor: pf, index: prevEntry.edgeIndex, segmentIndex: prevEntry.seg.segmentIndex });
      return;
    }
  ...
  const applyAdjustmentsUpdate = (
    ...
    if (bothCascadeReady) {
      const key = `${edgeIndex}-${segmentIndex}`;
      // S-5b-2: isF2 二枝を融合。record キーは floor 直用（both では primaryFloor/subFloor と恒等）。
      if (floor !== topFloor && !layoutByFloor) return; // 旧 下位階(else)枝のガード
      const cur = (bothmodeAdjustmentsByFloor[floor] ?? {})[key] ?? DEFAULT_EDGE_ADJUSTMENT;
      const next = updater(cur);
      if (next === null) return;

      const newAdjustments = { ...(bothmodeAdjustmentsByFloor[floor] ?? {}), [key]: next };
      setBothmodeAdjustmentsByFloor(prev => ({ ...prev, [floor]: newAdjustments }));
      // S5-d cascade で全階再計算（調整後、下位階も追従）
      const res = computeCascadeLayout(
        cascadeInput.buildings,
        cascadeInput.distances,
        cascadeInput.topStart!, enabledSizes, priorityConfig,
        bothmodeSelectionsByFloor,
        { ...bothmodeAdjustmentsByFloor, [floor]: newAdjustments },
        band,
      );
      setLayoutByFloor(res);
  ...
  return (
    <div className="fixed inset-0 modal-overlay flex items-end sm:items-center justify-center z-50" onClick={(showConflictConfirm || activeEdge !== null) ? undefined : onClose}>
      ...
      {/* S-2f-d改修: 帯不履行時の方向別・範囲提案ダイアログ（計算入口で1回だけ） */}
      {showBandProposal && (() => {
        const { orig, props } = showBandProposal;
        const pick = (p: { lo: number; hi: number }) => {
          setShowBandProposal(null);
          setRangeDist({ lo: p.lo, hi: p.hi });
          handleCalc({ lo: p.lo, hi: p.hi, mode: 'center' }); // 提案帯は center 前提
        };
        const rows: { arrow: string; label: string; band: { lo: number; hi: number } }[] = [];
        if (props.expandUp) rows.push({ arrow: '→', label: '上へ拡張', band: props.expandUp });
        if (props.expandDown) rows.push({ arrow: '←', label: '下へ拡張', band: props.expandDown });
        if (props.expandBoth) rows.push({ arrow: '↕', label: '両側へ拡張', band: props.expandBoth });
        return (
          <div className="fixed inset-0 z-[70] flex items-center justify-center px-6">
            <div className="absolute inset-0 bg-black/50" onClick={() => setShowBandProposal(null)} />
            <div className="relative bg-dark-surface border border-dark-border rounded-2xl p-5 w-full max-w-sm shadow-2xl">
              <p className="font-bold text-sm mb-1">{orig.lo}〜{orig.hi}mm では割り付けできません</p>
              <p className="text-xs text-dimension leading-relaxed mb-3">
                この範囲内の離れだけでは一周が割り切れません。下記の範囲なら割付可能です。
              </p>
              <div className="flex flex-col gap-2 mb-3">
                {rows.map((r) => (
                  <button
                    key={r.arrow}
                    onClick={() => pick(r.band)}
                    className="w-full py-2.5 px-3 bg-dark-bg border border-accent/60 text-canvas rounded-xl text-sm text-left hover:bg-accent/10 transition-colors flex items-center gap-2"
                  >
                    <span className="text-accent font-bold text-base w-4 text-center">{r.arrow}</span>
                    <span className="font-mono font-bold">{r.band.lo}〜{r.band.hi}mm</span>
                    <span className="text-[11px] text-dimension">なら割付可能です（{r.label}）</span>
                  </button>
                ))}
              </div>
              <button
                onClick={() => setShowBandProposal(null)}
                className="w-full py-2.5 border border-dark-border text-dimension font-bold rounded-xl text-sm"
              >
                このまま面ごとに調整する
              </button>
            </div>
